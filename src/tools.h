...
#define CHAR_STR_COPY(dest,src) dest = (char*)malloc(strlen(src)+1);if(dest == NULL){printf("Failed malloc");exit(ERR_NOALLOC);}memcpy(dest, src, strlen(src));dest[strlen(src)]=0;
